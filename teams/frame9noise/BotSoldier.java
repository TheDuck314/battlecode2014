	...
	public void turn() throws GameActionException {
		...
		if (attackTarget != null) {
			...
			if (rc.isActive()) {
				...
				if (ourPastrs.length > 0 && here.distanceSquaredTo(ourPastrs[0]) > 5) {
					Direction dir = here.directionTo(ourPastrs[0]);
					if (rc.canMove(dir)) {
						rc.sneak(dir);
						return;
					}
				} else {
					int offset = FastRandom.randInt(8);
					for (int i = 8; i-- > 0;) {
						Direction dir = Direction.values()[(offset + i) % 8];
						if (rc.canMove(dir)) {
							MapLocation dest = here.add(dir);
							if (ourPastrs.length == 0 || dest.distanceSquaredTo(ourPastrs[0]) <= 5) {
								rc.sneak(dir);
								return;
							}
						}
