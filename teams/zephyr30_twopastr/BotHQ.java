...
public class BotHQ extends Bot {
	...
	protected static void init(RobotController theRC) throws GameActionException {
		...
		centralRallyPoint = new MapLocation((ourHQ.x + theirHQ.x) / 2, (ourHQ.y + theirHQ.y) / 2);
	...
	private static void doFirstTurn() throws GameActionException {
		...
		if (Strategy.active == Strategy.ONE_PASTR || Strategy.active == Strategy.ONE_PASTR_SUPPRESSOR || Strategy.active == Strategy.SCATTER
			...
			int xmin = bestPastrLocations[0].x - 2;
			int xmax = bestPastrLocations[0].x + 2;
			int ymin = bestPastrLocations[0].y - 2;
			int ymax = bestPastrLocations[0].y + 2;
			for(int x = xmin; x <= xmax; x++) {
				for(int y = ymin; y <=  ymax; y++) {
					totalCowGrowth += cowGrowth[x][y];
				}
			}
			Debug.indicate("pastr", 0, "Total cow growth near pastr 1 = " + totalCowGrowth);
			if(numPastrLocations == 2) {
				totalCowGrowth = 0;
				xmin = bestPastrLocations[1].x - 2;
				xmax = bestPastrLocations[1].x + 2;
				ymin = bestPastrLocations[1].y - 2;
				ymax = bestPastrLocations[1].y + 2;
				for(int x = xmin; x <= xmax; x++) {
					for(int y = ymin; y <=  ymax; y++) {
						totalCowGrowth += cowGrowth[x][y];
					}
				}
				Debug.indicate("pastr", 1, "Total cow growth near pastr 2 = " + totalCowGrowth);
			}
