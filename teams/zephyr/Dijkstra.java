...
public class Dijkstra {
	...
	private static void initDijsktra(MapLocation dest) {
	...
	private static MapLocation priorityQueuePop() {
		...
		double bubbleDownLocDist = bestDist[bubbleDownLoc.x][bubbleDownLoc.y];
		bubbleDown(1, bubbleDownLoc, bubbleDownLocDist);
		heapSize--;
		numPopped++;
		return ret;
	}

	private static void bubbleDown(int node, MapLocation nodeLoc, double nodeBestDist) {
		...
		if (rightChildNode > heapSize) {
			...
			double leftChildDist = bestDist[leftChildLoc.x][leftChildLoc.y];
			if (nodeBestDist <= leftChildDist) {
		...
		// both children exist
		MapLocation leftChildLoc = heap[leftChildNode];
		double leftChildBestDist = bestDist[leftChildLoc.x][leftChildLoc.y];
		MapLocation rightChildLoc = heap[rightChildNode];
		double rightChildBestDist = bestDist[rightChildLoc.x][rightChildLoc.y];

		int bestChildNode;
		MapLocation bestChildLoc;
		double bestChildBestDist;
		if (leftChildBestDist < rightChildBestDist) {
			bestChildNode = leftChildNode;
			bestChildLoc = leftChildLoc;
			bestChildBestDist = leftChildBestDist;
		} else {
			bestChildNode = rightChildNode;
			bestChildLoc = rightChildLoc;
			bestChildBestDist = rightChildBestDist;
		}

		if (nodeBestDist < bestChildBestDist) {
			...
			bubbleDown(bestChildNode, nodeLoc, nodeBestDist);
		}
	}

	private static void priorityQueuePush(MapLocation loc) {
		// System.out.println("pushing location " + loc.toString());
		bubbleUp(heapSize + 1, loc, bestDist[loc.x][loc.y]);
		heapSize++;
	}

	private static void bubbleUp(int node, MapLocation nodeLoc, double nodeBestDist) {
		...
		double parentBestDist = bestDist[parentLoc.x][parentLoc.y];

		if (nodeBestDist < parentBestDist) {
			// System.out.println("swapping with parent node " + parentNode);
			setHeapNode(node, parentLoc);
			bubbleUp(parentNode, nodeLoc, nodeBestDist);
		} else {
	...
	public static void work(MapLocation dest, int priority, int bytecodeLimit) throws GameActionException {
		...
		while (heapSize != 0 && Clock.getBytecodeNum() < bytecodeLimit) {
			...
			for (int i = 8; i-- > 0;) {
				...
				if (bestDist[x][y] == 0) {
					...
					bestDist[x][y] = newLocDist;
					bestDir[x][y] = dirs[i];
					bubbleUp(heapNode[x][y], newLoc, newLocDist);
				}
			}
		}
		...
		Debug.indicate("path", 0, "Dijkstra num popped = " + numPopped);
	}
