...
import anatid16_shotgun.Nav.Sneak;
...
public class BotSoldier extends Bot {
	...
	public void turn() throws GameActionException {
		...
		Debug.indicate("micro", 0, "stance = " + stance.toString());
		...
		int shotgunClass = (spawnOrder - 1) % 4;
		MapLocation dest = MessageBoard.SHOTGUN_PASTR_LOCATIONS.readShotgunPastrLocation(shotgunClass);
		if (dest != null) {
			if (here.isAdjacentTo(dest) && rc.senseNearbyGameObjects(Robot.class, dest, 2, us).length != 1) {
				constructAndAdvertiseNoiseTower(dest);
				return;
			}
			if (here.equals(dest)) {
			...
			Nav.goTo(dest, Nav.Sneak.NO, Nav.Engage.NO);
		}
		return;
	}
	...
	private void fight() throws GameActionException {
		Debug.indicate(
				"micro",
				0,
				String.format("numVisibleEnemies = %d; numAttackableEnemies = %d; stance = %s", visibleEnemies.length, attackableEnemies.length,
						stance.toString()));
