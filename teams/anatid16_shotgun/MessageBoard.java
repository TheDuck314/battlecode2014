...
public enum MessageBoard {
	...
	SHOTGUN_PASTR_LOCATIONS(GameConstants.BROADCAST_MAX_CHANNELS - 13);
	...
	public static void setDefaultChannelValues() throws GameActionException {
		...
		for(int i = 0; i < 4; i++) SHOTGUN_PASTR_LOCATIONS.writeShotgunPastrLocation(i, null);
	}
	...
	public void writeShotgunPastrLocation(int which, MapLocation loc) throws GameActionException {
		int data = (loc == null ? -999 : loc.x * GameConstants.MAP_MAX_WIDTH + loc.y);
		rc.broadcast(channel + which, data);
	}
	
	public MapLocation readShotgunPastrLocation(int which) throws GameActionException {
		int data = rc.readBroadcast(channel + which);
		if(data == -999) return null;
		else return new MapLocation(data / GameConstants.MAP_MAX_WIDTH, data % GameConstants.MAP_MAX_WIDTH);
	}
}
