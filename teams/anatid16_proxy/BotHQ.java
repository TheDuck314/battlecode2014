...
public class BotHQ extends Bot {
	...
	int towerBuilderId = -1;
	int pastrBuilderId = -1;
	boolean towerBuilderStillAlive;
	boolean pastrBuilderStillAlive;
	boolean pastrBuildTriggered = false;
	...
	private void updateStrategicInfo() throws GameActionException {
		...
		numAlliedSoldiers = 0;
		towerBuilderStillAlive = false;
		pastrBuilderStillAlive = false;
		towerBuilderId = MessageBoard.TOWER_BUILDER_ROBOT_ID.readInt();
		pastrBuilderId = MessageBoard.PASTR_BUILDER_ROBOT_ID.readInt();
		...
		for (int i = allAlliedRobots.length; i-- > 0;) {
			...
			int id = ally.getID();
			if (id == towerBuilderId || info.type == RobotType.NOISETOWER) towerBuilderStillAlive = true;
			if (id == pastrBuilderId || info.type == RobotType.PASTR) pastrBuilderStillAlive = true;
		}
	...
	private void directStrategyProxy() throws GameActionException {
		Debug.indicate("proxy", 0, String.format("tower builder = (%d, %s), pastr builder = (%d, %s)", towerBuilderId, towerBuilderStillAlive ? "true"
				: "false", pastrBuilderId, pastrBuilderStillAlive ? "true" : "false"));

		MapLocation rally = null;
		boolean beAggressive = false;
		if (theirPastrs.length == 0) {
			if (pastrBuildTriggered) {
				rally = computedBestPastrLocation;
			} else {
		...
		if (theirPastrs.length >= 1 || pastrBuildTriggered || Clock.getRoundNum() > 1300) {
			pastrBuildTriggered = true;

			if (computedBestPastrLocation == null) {
				computePastrScores();
				computeBestPastrLocation();
				MessageBoard.BEST_PASTR_LOC.writeMapLocation(computedBestPastrLocation);
				return;
			...
			int nextSpawnOrder = MessageBoard.SPAWN_COUNT.readInt() + 1;
			if (!towerBuilderStillAlive) {
				MessageBoard.TOWER_BUILDER_SPAWN_ORDER.writeInt(nextSpawnOrder);
				Debug.indicate("proxy", 1, String.format("tower builder spawn order = %d", nextSpawnOrder));
				nextSpawnOrder++;
			}
			if (!pastrBuilderStillAlive) {
				MessageBoard.PASTR_BUILDER_SPAWN_ORDER.writeInt(nextSpawnOrder);
				Debug.indicate("proxy", 2, String.format("pastr builder spawn order = %d", nextSpawnOrder));
			}
		}
	...
	private void pathfindWithSpareBytecodes() throws GameActionException {
		...
		if (rallyLoc != null) {
			Bfs.work(rallyLoc, rc, Bfs.PRIORITY_HIGH, bytecodeLimit);
